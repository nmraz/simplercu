...
static inline void rcu_read_lock(void) {
    ...
    // Barrier A: Pairs with E in `synchronize_rcu`.
    // Ensures that whenever a grace period initiator's read of this thread's
    // `read_lock_nesting` reads-before the increment above, this thread will
    // observe all accesses performed before the grace period.
    asymm_fence_seq_cst_light();
}

static inline void rcu_read_unlock(void) {
    // Barrier B: Pairs with F in `synchronize_rcu`.
    ...
    // thread as quiescent, it will observe any accesses in the preceding
    ...
    if (nesting == 1) {
        ...
        // Barrier C: Pairs with E in `synchronize_rcu`.
